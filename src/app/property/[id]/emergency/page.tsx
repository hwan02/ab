...
const CATEGORY_CONFIG: Record<
  ...
  { label: string; bgColor: string; textColor: string; iconPath: string }
> = {
  hospital: {
    label: "병원",
    bgColor: "bg-red-50",
...
export default async function EmergencyPage({ params }: EmergencyPageProps) {
  ...
  return (
    <div className="px-4 py-4">
      <div className="mb-4">
        <h2 className="text-xl font-bold text-gray-900">긴급 연락처</h2>
        <p className="mt-1 text-sm text-gray-500">
          긴급 상황 시 아래 연락처를 이용하세요
        </p>
      </div>

      {/* Quick Dial */}
      <div className="mb-6 grid grid-cols-3 gap-2">
        <QuickDialButton number="119" label="응급/소방" color="bg-red-500" />
        <QuickDialButton number="112" label="경찰" color="bg-blue-500" />
        <QuickDialButton number="1339" label="의료상담" color="bg-green-500" />
      </div>

      {groupedContacts.length === 0 ? (
        <EmptyState
          title="등록된 연락처가 없습니다"
          description="호스트가 긴급 연락처를 등록하면 여기에 표시됩니다."
          icon={
            <svg
